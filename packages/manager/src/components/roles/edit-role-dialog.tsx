...
export function EditRoleDialog({ id, open, onOpenChange }: EditRoleDialogProps) {
  const navigate = useNavigate()
  const { token, user: getUser } = useSelfStore()
  const { data, list: listRoles, populateSingle } = useRoleStore()
  ...
  if (open && formData.id === 0 && id != null) {
    const currentRole = data[id]
    if (currentRole) {
      setFormData({
        id: currentRole.id,
        name: currentRole.name,
        order: String(currentRole.order),
        permissions: buildPermissions(currentRole.permissions),
      })
    }
  }
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (!hasError) {
        setIsLoading(true)
        try {
          const role = await updateRole(token || '', {
            id: formData.id,
